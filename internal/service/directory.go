...
// Generate writes index.md as a combined homepage and page catalog,
// grouped by top-level tag domain. Returns the file path and page count.
func (s *DirectoryService) Generate() (string, int, error) {
	...
	b.WriteString("---\n")
	b.WriteString("title: Home Wiki\n")
	b.WriteString("tags:\n")
	b.WriteString("  - meta\n")
	b.WriteString("date: 2026-04-06\n")
	b.WriteString("description: Shared knowledge base for the Edwards homelab\n")
	b.WriteString("---\n\n")
	b.WriteString("A shared knowledge base for the Edwards homelab — maintained by humans and AI agents.\n\n")
	b.WriteString("See [[meta/schema]] for the operating manual. See [[meta/log]] for recent activity.\n\n")
	_, _ = fmt.Fprintf(&b, "**%d pages** across %d categories. ", len(entries), len(groupNames))
	b.WriteString("Auto-generated by `wiki-server directory --generate`.\n")
	...
	indexFile := filepath.Join(s.vault.Dir, "index.md")
	if err := os.WriteFile(indexFile, []byte(b.String()), 0o644); err != nil {
		return "", 0, err
	}

	return indexFile, len(entries), nil
}

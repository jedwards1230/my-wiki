...
func registerTools(
	...
	s.AddTool(
		mcp.NewTool("write",
			...
			mcp.WithString("path",
				mcp.Required(),
				mcp.Description("Relative path for the page (e.g., project/my-project). The .md extension is added if omitted. The server slugifies the filename segment (lowercase, hyphenated, smart-quotes/punctuation stripped) — provide the folder and an approximate name; the canonical on-disk path is server-assigned."),
			),
	...
	s.AddTool(
		mcp.NewTool("move",
			...
			mcp.WithString("destination",
				mcp.Required(),
				mcp.Description("New relative path for the page (e.g., project/new-name). The .md extension is added if omitted. The filename segment is slugified server-side, so the final destination is canonical regardless of casing/punctuation you supply."),
			),
		),

...
| `/raw/{path}` | raw/ markdown → full compiled wiki page (browser) or verbatim bytes (`?raw=1` / non-HTML Accept); raw/ assets → native bytes; directories → generated `index.md` landing (browser), gallery fallback for asset-only dirs, plain autoindex for agents |
